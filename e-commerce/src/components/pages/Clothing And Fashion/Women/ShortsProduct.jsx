...
export const ShortsProduct = () => {
  ...
  const getData = async () => {
    const { data, error } = await supabase.from("short").select().eq("id", id);
    if (error) {
  ...
  const getVintage = async () => {
    const { data, error } = await supabase.from("Tshirt").select();
  ...
  return (
    <div>
      ...
      <div className=" pt-24 md:pt-7 px-5">
        {shirt.map((item) => {
          return (
            <div className="md:flex gap-5" key={item.id}>
              <img
                src={item.image}
      ...
      <div className="shadow  py-3 mt-5 overflow-x-hidden">
