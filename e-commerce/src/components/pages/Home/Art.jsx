...
export const Art = () => {
  ...
  const getData = async () => {
    const { data, error } = await supabase.from("art").select().eq("id", id);
    if (error) {
  ...
  const getWatch = async () => {
    const { data, error } = await supabase.from("artProduct").select();
        ...
        {art.map((item) => {
          return (
            <div className="md:flex gap-5 bg-white p-2" key={item.id}>
              ...
              />
              <div>
                <h1 className="font-bold text-xl max-w-xs">{item.title}</h1>
