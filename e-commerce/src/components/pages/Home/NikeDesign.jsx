...
export const NikeDesign = () => {
  ...
  const getData = async () => {
    const { data, error } = await supabase.from("nike").select().eq("id", id);
    if (error) {
  ...
  const getVintage = async () => {
    const { data, error } = await supabase.from("nikeProduct").select();
